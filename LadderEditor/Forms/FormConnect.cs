...
using Devinno.Tools;
using LadderEditor.Communication;
using System;
...
namespace LadderEditor.Forms
{
    public partial class FormConnect : DvForm
    {
        ...
        public FormConnect()
        {
            ...
            tree.SelectionMode = Devinno.Forms.Controls.ItemSelectionMode.SINGLE;
            tree.NodeClick += (o, s) => { if (s.Node.Tag is ConnItem) inAddr.Value = ((ConnItem)s.Node.Tag).Address; };
            #endregion
                ...
                if (str != null)
                {
                    ...
                    if (strs.Length == 4)
                    {
                        ...
                        foreach (var v in lstConn)
                            tree.Nodes.Add(new Devinno.Forms.Controls.TreeViewNode(v.Name + " - " + v.Address) { Tag = v });

                        if (ret != null)
                            foreach (var k in ret.Keys)
                                tree.Nodes.Add(new Devinno.Forms.Controls.TreeViewNode(k) { Tag = new ConnItem() { Name = "Unknown", Address = k } });
            ...
            btnPlus.ButtonClick += (o, s) => {
                ...
                ls.Add("Name", new InputBoxInfo() { Alias = "연결명" });
                ls.Add("Address", new InputBoxInfo() { Alias = "주소" });
            ...
            btnMinus.ButtonClick += (o, s) => {
                ...
                {
                    foreach (var v in tree.SelectedItems) tree.Nodes.Remove(v);
                    tree.Invalidate();
